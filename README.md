...
同时，Kenny认为XShareChain不属于任何国家、组织和个人，它归属于全球网民。
...
* 当某个网民在XShareChain上链（成功提交一个）闲置物品，会获得n个XShareCoin的奖励；
* 奖励的数量由我们开发的智能算法自行确定，不受任何人影响。
* 该智能算法依靠图像识别、语义分析、以及大数据分析，会确定该物品的价值，然后发放相当的XShareCoin；
* 该智能算法是开源的，由XShareChain用户参与共建，并由自治委员会授权我们实现；
* 随着时间推移，该闲置物品的价值会自动逐渐降低（折旧），体现在以XShareCoin的标价降低；
